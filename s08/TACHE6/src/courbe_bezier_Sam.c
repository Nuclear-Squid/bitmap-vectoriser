...
Point valeur_courbe_2(PointList Courbe ,float t){
    return add_point(add_point(prod_scal_point(Courbe.head->pos,(1-t)*(1-t)),prod_scal_point(i_element(Courbe, 1),(1-t)*2*t)),prod_scal_point(i_element(Courbe, 2),t*t));
}
...
PointList simplification_d_p_bezier2(PointList CONST,int j1,int j2,float d){
    float n = j2 -j1;
    PointList B = approx_bezier2(CONST ,j1,j2);
    float dmax = 0;
    int k = j1;
    for(int j= j1 + 1; j <= j2;j++){
        float i= j-j1;
        float ti = i/n;
        float dj = distance_point_bezier(B,ti,i_element(CONST, j));
        if(dmax < dj){
            dmax = dj;
            k = j;
        }
